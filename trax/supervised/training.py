...
class Loop:
  ...
  def init_and_run(self, n_steps=1):
    """Runs this training loop for n steps, starting from random initialization.

    Args:
      n_steps: Stop training after completing n steps.
    """
    model, task, evals = self._model, self._task, self._evals
  ...
  def _run_one_step(self):
    ...
    model, task, optimizer = self._model, self._task, self._task.optimizer
    weights, opt_slots = model.weights, optimizer.slots
    opt_params = optimizer._init_opt_params  # pylint: disable=protected-access
    batch = task.next_batch()
    model_with_loss = tl.Serial(model, task.loss_layer)
    ...
    model.weights, optimizer.slots = optimizer.tree_update(
        self.current_step, gradients, weights, opt_slots, opt_params)

  def _log_step(self, msg):
    """Logs message, labeled with the current training step number."""
    print(f'\nStep {self.current_step}: {msg}')
